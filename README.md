...
- Each subproject has its own `requirements.txt`; install from that folder, e.g.  
  `pip install -r text-chunking/requirements.txt`, `pip install -r spacy/requirements.txt`, `pip install -r gliner/requirements.txt`, `pip install -r llm-based-extraction/requirements.txt`, or `pip install -r jira/story-extractor/requirements.txt`.
